...
document.addEventListener('DOMContentLoaded', () => {
  ...
  heroTimeline
    ...
    .from('.photo-frame',{ opacity: 0, y: 30, duration: 0.7 },  '-=0.6');

  document.querySelectorAll('section').forEach((section) => {
    const head = section.querySelector('.section-head');
    if (head) {
      gsap.fromTo(head, { opacity: 0, y: 30 }, {
    ...
    const grids = section.querySelectorAll(
      '.focus-grid, .proj-grid, .labs-grid, .resume-grid, .skills-grid, .highlight-list, .timeline, .cv-referee-grid'
    );
    grids.forEach(grid => {
    ...
    if (aboutCopy) {
      gsap.fromTo(aboutCopy, { opacity: 0, y: 30 }, {
        opacity: 1, y: 0, duration: 0.6, ease: 'power3.out',
    ...
    if (contactCard) {
      gsap.fromTo(contactCard, { opacity: 0, y: 30 }, {
        opacity: 1, y: 0, duration: 0.6, ease: 'power3.out',
